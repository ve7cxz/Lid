...
sub process {
  ...
  switch ($data->type) {
    ...
    case "connect" {
      $self->debug("Connection request from ".$data->source.".");
      $self->connect($data);
    }
    case "disconnect" {
      $self->debug("Disconnection request from ".$data->source.".");
...
sub connect {
  ...
  $self->debug("connect() called.");

  $self->client($msg->source);
  $self->connected(1);

  $self->debug("Client ".$msg->source." is now connected to console ".$self->id.".");
  $self->out($self->create_message($msg->source, "connect_ok"));

  $self->debug("connect() finished.");
}
